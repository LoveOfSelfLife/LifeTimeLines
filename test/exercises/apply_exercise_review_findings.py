...
def backup_exercise_table():
    ...
    for exercise in exercises:
        ...
        transformed_exercise = ExerciseEntityBackup(exercise)
        updated_exercises.append(transformed_exercise)
    es.upsert_items(updated_exercises)
    
def update_exercise_table(exercises):
    es = EntityStore()
    es.upsert_items(exercises)
        
def load_exercises_and_reviews():
    es = EntityStore()
    ...
    print(f"number of exercises: {len(exercise_map)}")

    exercise_review_map = {}    
    for r in es.list_items(ExerciseReviewEntity()):
        exercise_review_map[r['id']] = r
    
    print(f"number of reviews: {len(exercise_review_map)}")
    
    return exercise_map, exercise_review_map
